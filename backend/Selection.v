...
Fixpoint sel_stmt (s: Cminor.stmt) : res stmt :=
  ...
  | Cminor.Scall optid sg fn args =>
      OK (match expr_is_addrof_ident fn with
      | None => Scall optid sg (inl _ (sel_expr fn)) (sel_exprlist args)
      | Some id  => Scall optid sg (inr _ id) (sel_exprlist args)
      end)
  | Cminor.Sbuiltin optid ef args =>
      OK (Sbuiltin optid ef (sel_exprlist args))
  | Cminor.Stailcall sg fn args => 
      OK (match expr_is_addrof_ident fn with
      | None => Stailcall sg (inl _ (sel_expr fn)) (sel_exprlist args)
      | Some id  => Stailcall sg (inr _ id) (sel_exprlist args)
      end)
  | Cminor.Sseq s1 s2 =>
...
Definition sel_program (p: Cminor.program) : res program :=
  do p' <- transform_partial_program sel_fundef p;
  assertion (check_helpers p');
  OK p'.

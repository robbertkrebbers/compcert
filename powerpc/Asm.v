...
Proof.
  intros ge.
  assert (LOAD1: forall ch i c j rs1 m1 rs2 m2,
    load1 ge ch i c j rs1 m1 = Next rs2 m2 -> Mem.forward m1 m2).
  { unfold load1; intros.
    destruct (Mem.loadv ch m1 _) eqn:?; inv H; eauto using Mem.forward_refl. }
  assert (LOAD2: forall ch i c j rs1 m1 rs2 m2,
    load2 ch i c j rs1 m1 = Next rs2 m2 -> Mem.forward m1 m2).
  { unfold load2; intros.
    destruct (Mem.loadv ch m1 _) eqn:?; inv H; eauto using Mem.forward_refl. }
  ...
  assert (STORE1: forall ch i c j rs1 m1 rs2 m2,
    store1 ge ch i c j rs1 m1 = Next rs2 m2 -> Mem.forward m1 m2).
  { unfold store1; intros.
    destruct (Mem.storev ch m1 _ _) eqn:?; inv H; eauto using Mem.storev_forward. }
  assert (STORE2: forall ch r r1 r2 rs1 m1 rs2 m2,
    store2 ch r r1 r2 rs1 m1 = Next rs2 m2 -> Mem.forward m1 m2).
  { unfold store2; intros.
    destruct (Mem.storev ch m1 _ _) eqn:?; inv H; eauto using Mem.storev_forward. }
  ...
  eauto using Mem.forward_trans, Mem.store_forward, Mem.alloc_forward.
Qed.

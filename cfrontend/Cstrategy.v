...
Inductive leftcontext: kind -> kind -> (expr -> expr) -> Prop :=
  ...
  | lctx_comma: forall k C sp e2 ty,
      leftcontext k RV C -> leftcontext k RV (fun x => Ecomma sp (C x) e2 ty)
...
Inductive estep: state -> trace -> state -> Prop :=
  ...
  | step_expr: forall f r k e m sq v ty,
      eval_simple_rvalue e m sq r v ->
      match r with Eval _ _ => False | _ => True end ->
      ty = typeof r ->
      estep (ExprState f r k e m sq)
         E0 (ExprState f (Eval v ty) k e m sq)

  | step_rvalof_volatile: forall f C l ty k e m sq b ofs t v,
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      deref_loc ge ty m sq b ofs t v ->
      ty = typeof l -> type_is_volatile ty = true ->
      estep (ExprState f (C (Evalof l ty)) k e m sq)
          t (ExprState f (C (Eval v ty)) k e m sq)

  | step_seqand_true: forall f C r1 r2 ty k e m sq v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      bool_val v (typeof r1) = Some true ->
      estep (ExprState f (C (Eseqand r1 r2 ty)) k e m sq)
         E0 (ExprState f (C (Eparen (Eparen r2 type_bool) ty)) k e m empty_seqs)
  | step_seqand_false: forall f C r1 r2 ty k e m sq v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      bool_val v (typeof r1) = Some false ->
      estep (ExprState f (C (Eseqand r1 r2 ty)) k e m sq)
         E0 (ExprState f (C (Eval (Vint Int.zero) ty)) k e m sq)

  | step_seqor_true: forall f C r1 r2 ty k e m sq v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      bool_val v (typeof r1) = Some true ->
      estep (ExprState f (C (Eseqor r1 r2 ty)) k e m sq)
         E0 (ExprState f (C (Eval (Vint Int.one) ty)) k e m sq)
  | step_seqor_false: forall f C r1 r2 ty k e m sq v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      bool_val v (typeof r1) = Some false ->
      estep (ExprState f (C (Eseqor r1 r2 ty)) k e m sq)
  ...
  | step_condition: forall f C r1 r2 r3 ty k e m sq v b,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      bool_val v (typeof r1) = Some b ->
      estep (ExprState f (C (Econdition r1 r2 r3 ty)) k e m sq)
         E0 (ExprState f (C (Eparen (if b then r2 else r3) ty)) k e m empty_seqs)

  | step_assign: forall f C l r ty k e m sq b ofs v v' t m' sq',
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      eval_simple_rvalue e m sq r v ->
      sem_cast v (typeof r) (typeof l) = Some v' ->
      assign_loc ge (typeof l) m sq b ofs v' t m' sq' ->
      ty = typeof l ->
      estep (ExprState f (C (Eassign l r ty)) k e m sq)
          t (ExprState f (C (Eval v' ty)) k e m' sq')

  | step_assignop: forall f C op l r tyres ty k e m sq b ofs v1 v2 v3 v4 t1 t2 m' sq' t,
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      deref_loc ge (typeof l) m sq b ofs t1 v1 ->
      eval_simple_rvalue e m sq r v2 ->
      sem_binary_operation op v1 (typeof l) v2 (typeof r) m = Some v3 ->
      sem_cast v3 tyres (typeof l) = Some v4 ->
      assign_loc ge (typeof l) m sq b ofs v4 t2 m' sq' ->
      ty = typeof l ->
      t = t1 ** t2 ->
      estep (ExprState f (C (Eassignop op l r tyres ty)) k e m sq)
          t (ExprState f (C (Eval v4 ty)) k e m' sq')

  | step_assignop_stuck: forall f C op l r tyres ty k e m sq b ofs v1 v2 t,
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      deref_loc ge (typeof l) m sq b ofs t v1 ->
      eval_simple_rvalue e m sq r v2 ->
      ...
      | Some v3 =>
          ...
          | None => True
          | Some v4 => forall t2 m' sq', ~(assign_loc ge (typeof l) m sq b ofs v4 t2 m' sq')
          end
      end ->
      ty = typeof l ->
      estep (ExprState f (C (Eassignop op l r tyres ty)) k e m sq)
  ...
  | step_postincr: forall f C id l ty k e m sq b ofs v1 v2 v3 t1 t2 m' sq' t,
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      deref_loc ge ty m sq b ofs t1 v1 ->
      sem_incrdecr id v1 ty = Some v2 ->
      sem_cast v2 (typeconv ty) ty = Some v3 ->
      assign_loc ge ty m sq b ofs v3 t2 m' sq' ->
      ty = typeof l ->
      t = t1 ** t2 ->
      estep (ExprState f (C (Epostincr id l ty)) k e m sq)
          t (ExprState f (C (Eval v1 ty)) k e m' sq')

  | step_postincr_stuck: forall f C id l ty k e m sq b ofs v1 t,
      leftcontext RV RV C ->
      eval_simple_lvalue e m sq l b ofs ->
      ...
      | Some v2 =>
          ...
          | None => True
          | Some v3 => forall t2 m' sq', ~(assign_loc ge (typeof l) m sq b ofs v3 t2 m' sq')
          end
      end ->
      ty = typeof l ->
      estep (ExprState f (C (Epostincr id l ty)) k e m sq)
  ...
  | step_comma: forall f C sp r1 r2 ty k e m sq v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r1 v ->
      ty = typeof r2 ->
      estep (ExprState f (C (Ecomma sp r1 r2 ty)) k e m sq)
         E0 (ExprState f (C r2) k e m (if sp then empty_seqs else sq))

  | step_paren: forall f C r ty k e m sq v1 v,
      leftcontext RV RV C ->
      eval_simple_rvalue e m sq r v1 ->
      sem_cast v1 (typeof r) ty = Some v ->
      estep (ExprState f (C (Eparen r ty)) k e m sq)
         E0 (ExprState f (C (Eval v ty)) k e m sq)

  | step_call: forall f C rf rargs ty k e m sq targs tres cconv vf vargs fd,
      leftcontext RV RV C ->
      classify_fun (typeof rf) = fun_case_f targs tres cconv ->
      eval_simple_rvalue e m sq rf vf ->
      eval_simple_list e m sq rargs targs vargs ->
      Genv.find_funct ge vf = Some fd ->
      type_of_fundef fd = Tfunction targs tres cconv ->
      estep (ExprState f (C (Ecall rf rargs ty)) k e m sq)
         E0 (Callstate fd vargs (Kcall f e C ty k) m)

  | step_builtin: forall f C ef tyargs rargs ty k e m sq vargs t vres m',
      leftcontext RV RV C ->
      eval_simple_list e m sq rargs tyargs vargs ->
      external_call ef ge vargs m t vres m' ->
      estep (ExprState f (C (Ebuiltin ef tyargs rargs ty)) k e m sq)
          t (ExprState f (C (Eval vres ty)) k e m' empty_seqs).
...
Lemma safe_imm_safe:
  forall f C a k e m sq K,
  safe (ExprState f (C a) k e m sq) ->
  context K RV C ->
  imm_safe ge e K a m sq.
Proof.
  intros. destruct (classic (imm_safe ge e K a m sq)); auto.
...
Lemma rred_kind:
  forall a m sq t a' m' sq', rred ge a m sq t a' m' sq' -> expr_kind a = RV.
Proof.
  induction 1; auto.
Qed.
...
Lemma imm_safe_kind:
  forall e k a m sq, imm_safe ge e k a m sq -> expr_kind a = k.
Proof.
  induction 1.
  auto.
  auto.
...
Definition invert_expr_prop (a: expr) (m: mem) (sq: seqs) : Prop :=
  ...
  | Evalof (Eloc b ofs ty') ty =>
      ty' = ty /\ exists t, exists v, deref_loc ge ty m sq b ofs t v
  ...
  | Eassign (Eloc b ofs ty1) (Eval v2 ty2) ty =>
      exists v, exists m', exists sq', exists t,
      ty = ty1 /\ sem_cast v2 ty2 ty1 = Some v /\ assign_loc ge ty1 m sq b ofs v t m' sq'
...
Lemma rred_invert:
  forall r m sq t r' m' sq', rred ge r m sq t r' m' sq' -> invert_expr_prop r m sq.
Proof.
  induction 1; red; auto.
  ...
  exists true; auto. exists false; auto.
  exists b; auto.
  exists v; exists m'; exists sq'; exists t; auto.
...
Proof.
  ...
  auto.
  auto.
  assert (invert_expr_prop (C e0) m sq).
...
Lemma eval_simple_steps:
   (forall a v, eval_simple_rvalue e m sq a v ->
    forall C, context RV RV C ->
    star Csem.step ge (ExprState f (C a) k e m sq)
                  E0 (ExprState f (C (Eval v (typeof a))) k e m sq))
/\ (forall a b ofs, eval_simple_lvalue e m sq a b ofs ->
    forall C, context LV RV C ->
    star Csem.step ge (ExprState f (C a) k e m sq)
                  E0 (ExprState f (C (Eloc b ofs (typeof a))) k e m sq)).
Proof.
...
Corollary eval_simple_rvalue_safe:
  forall C a v,
  eval_simple_rvalue e m sq a v ->
  context RV RV C -> safe (ExprState f (C a) k e m sq) ->
  safe (ExprState f (C (Eval v (typeof a))) k e m sq).
Proof.
...
Corollary eval_simple_lvalue_safe:
  forall C a b ofs,
  eval_simple_lvalue e m sq a b ofs ->
  context LV RV C -> safe (ExprState f (C a) k e m sq) ->
  safe (ExprState f (C (Eloc b ofs (typeof a))) k e m sq).
Proof.
...
Lemma simple_can_eval:
  forall a from C,
  simple a = true -> context from RV C -> safe (ExprState f (C a) k e m sq) ->
  match from with
  | LV => exists b, exists ofs, eval_simple_lvalue e m sq a b ofs
  | RV => exists v, eval_simple_rvalue e m sq a v
  end.
Proof.
Ltac StepL REC C' a :=
  ...
  assert (S: safe (ExprState f (C' (Eloc b ofs (typeof a))) k e m sq)) by 
...
Ltac StepR REC C' a :=
  ...
  assert (S: safe (ExprState f (C' (Eval v (typeof a))) k e m sq)) by 
...
Lemma simple_can_eval_rval:
  forall r C, 
  simple r = true -> context RV RV C -> safe (ExprState f (C r) k e m sq) ->
  exists v, eval_simple_rvalue e m sq r v
        /\ safe (ExprState f (C (Eval v (typeof r))) k e m sq).
Proof.
...
Lemma simple_can_eval_lval:
  forall l C, 
  simple l = true -> context LV RV C -> safe (ExprState f (C l) k e m sq) ->
  exists b, exists ofs, eval_simple_lvalue e m sq l b ofs
         /\ safe (ExprState f (C (Eloc b ofs (typeof l))) k e m sq).
Proof.
...
Lemma eval_simple_list_implies:
  forall rl tyl vl,
  eval_simple_list e m sq rl tyl vl -> 
...
Lemma eval_simple_list_steps:
  ...
  star Csem.step ge (ExprState f (C rl) k e m sq)
                E0 (ExprState f (C (rval_list vl rl)) k e m sq).
Proof.
  induction 1; intros. 
(* nil *)
  apply star_refl.
...
Lemma simple_list_can_eval:
  ...
  simplelist rl = true ->
  contextlist' C ->
  safe (ExprState f (C rl) k e m sq) ->
...
Definition simple_side_effect (r: expr) : Prop :=
  ...
  | Ecomma _ r1 r2 _ => simple r1 = true
...
Lemma decompose_expr:
  (forall a from C,
   context from RV C -> safe (ExprState f (C a) k e m sq) ->
       simple a = true
    \/ exists C', exists a', a = C' a' /\ simple_side_effect a' /\ leftcontext RV from C')
/\(forall rl C,
   contextlist' C -> safe (ExprState f (C rl) k e m sq) ->
       simplelist rl = true
...
(* comma *)
  Kind. Rec H RV C (fun x => Ecomma sp x r2 ty). Base.
(* call *)
...
(* assignop stuck *)
  ...
  red; intros. exploit imm_safe_inv; eauto. simpl. intros [v4' [m' [sq' [t' [A [B D]]]]]].
...
(* postincr *)
  ...
  left; apply step_rred; auto. econstructor; eauto.
  eapply star_left.
  left; apply step_rred with (C := fun x =>
    C (Ecomma false (Eassign (Eloc b ofs (typeof l)) x (typeof l)) (Eval v1 (typeof l)) (typeof l))); eauto.
  econstructor. instantiate (1 := v2). destruct id; assumption. 
  eapply star_left.
  left; apply step_rred with (C := fun x => C (Ecomma false x (Eval v1 (typeof l)) (typeof l))); eauto.
  econstructor; eauto.
  apply star_one.
  left; apply step_rred; auto. econstructor; eauto. 
  reflexivity. reflexivity. reflexivity. traceEq.
(* postincr stuck *)
  ...
  econstructor; eauto.
  apply star_one.
  left; eapply step_stuck with (C := fun x => C (Ecomma false x (Eval v1 (typeof l)) (typeof l))); eauto.
  red; intros. exploit imm_safe_inv; eauto. simpl. intros [v3 [m' [sq' [t' [A [B D]]]]]].
  rewrite B in H3. eelim H3; eauto.
  reflexivity.
  apply star_one.
  left; eapply step_stuck with (C := fun x =>
    C (Ecomma false (Eassign (Eloc b ofs (typeof l)) x (typeof l)) (Eval v1 (typeof l)) (typeof l))); eauto.
...
(* comma *)
  eapply plus_right.
  eapply eval_simple_rvalue_steps with (C := fun x => C(Ecomma sp x r2 (typeof r2))); eauto.
  left; apply step_rred; eauto. econstructor; eauto. auto.
(* paren *)
...
Lemma can_estep:
  ...
  exists t, exists S, estep (ExprState f a k e m sq) t S.
Proof.
  intros. destruct (decompose_topexpr f k e m sq a H) as [A | [C [b [P [Q R]]]]].
(* simple expr *)
  exploit (simple_can_eval f k e m sq a RV (fun x => x)); auto. intros [v P].
  econstructor; econstructor; eapply step_expr; eauto. 
(* side effect *)
  clear H0. subst a. red in Q. destruct b; try contradiction. 
(* valof volatile *)
  destruct Q.
  exploit (simple_can_eval_lval f k e m sq b (fun x => C(Evalof x ty))); eauto.
  intros [b1 [ofs [E1 S1]]].
...
(* seqand *)
  exploit (simple_can_eval_rval f k e m sq b1 (fun x => C(Eseqand x b2 ty))); eauto.
  intros [v1 [E1 S1]].
...
(* seqor *)
  exploit (simple_can_eval_rval f k e m sq b1 (fun x => C(Eseqor x b2 ty))); eauto.
  intros [v1 [E1 S1]].
  exploit safe_inv. eexact S1. eauto. simpl. intros [b BV].
  destruct b.
...
(* condition *)
  exploit (simple_can_eval_rval f k e m sq b1 (fun x => C(Econdition x b2 b3 ty))); eauto.
  intros [v1 [E1 S1]].
  exploit safe_inv. eexact S1. eauto. simpl. intros [b BV].
  econstructor; econstructor. eapply step_condition; eauto. 
(* assign *)
  destruct Q.
  exploit (simple_can_eval_lval f k e m sq b1 (fun x => C(Eassign x b2 ty))); eauto.
  intros [b [ofs [E1 S1]]].
  exploit (simple_can_eval_rval f k e m sq b2 (fun x => C(Eassign (Eloc b ofs (typeof b1)) x ty))); eauto.
  intros [v [E2 S2]].
  exploit safe_inv. eexact S2. eauto. simpl. intros [v' [m' [sq' [t [A [B D]]]]]].
  econstructor; econstructor; eapply step_assign; eauto. 
(* assignop *)
  destruct Q.
  exploit (simple_can_eval_lval f k e m sq b1 (fun x => C(Eassignop op x b2 tyres ty))); eauto.
  intros [b [ofs [E1 S1]]].
  exploit (simple_can_eval_rval f k e m sq b2 (fun x => C(Eassignop op (Eloc b ofs (typeof b1)) x tyres ty))); eauto.
  intros [v [E2 S2]].
  ...
  destruct (classic (exists t2, exists m', exists sq', assign_loc ge (typeof b1) m sq b ofs v4 t2 m' sq')).
  destruct H2 as [t2 [m' [sq' D]]].
  econstructor; econstructor; eapply step_assignop; eauto.
  econstructor; econstructor; eapply step_assignop_stuck; eauto. 
  rewrite Heqo. rewrite Heqo0. intros; red; intros. elim H2. exists t2; exists m'; exists sq'; auto.
  econstructor; econstructor; eapply step_assignop_stuck; eauto. 
  rewrite Heqo. rewrite Heqo0. auto.
  econstructor; econstructor; eapply step_assignop_stuck; eauto. 
  rewrite Heqo. auto.
(* postincr *)
  exploit (simple_can_eval_lval f k e m sq b (fun x => C(Epostincr id x ty))); eauto.
  intros [b1 [ofs [E1 S1]]].
  ...
  destruct (classic (exists t2, exists m', exists sq', assign_loc ge ty m sq b1 ofs v3 t2 m' sq')).
  destruct H0 as [t2 [m' [sq' D]]].
  econstructor; econstructor; eapply step_postincr; eauto.
  econstructor; econstructor; eapply step_postincr_stuck; eauto. 
  rewrite Heqo. rewrite Heqo0. intros; red; intros. elim H0. exists t2; exists m'; exists sq'; congruence.
  econstructor; econstructor; eapply step_postincr_stuck; eauto. 
  rewrite Heqo. rewrite Heqo0. auto.
  econstructor; econstructor; eapply step_postincr_stuck; eauto. 
  rewrite Heqo. auto.
(* comma *)
  exploit (simple_can_eval_rval f k e m sq b1 (fun x => C(Ecomma sp x b2 ty))); eauto.
  intros [v1 [E1 S1]].
  exploit safe_inv. eexact S1. eauto. simpl. intros EQ.
  econstructor; econstructor; eapply step_comma; eauto.
(* call *)
  destruct Q.
  exploit (simple_can_eval_rval f k e m sq b (fun x => C(Ecall x rargs ty))); eauto.
  intros [vf [E1 S1]].
  pose (C' := fun x => C(Ecall (Eval vf (typeof b)) x ty)).
  assert (contextlist' C'). unfold C'; eapply contextlist'_call with (rl0 := Enil); auto.
  exploit (simple_list_can_eval f k e m sq rargs C'); eauto.
  intros [vl E2].
  exploit safe_inv. 2: eapply leftcontext_context; eexact R.
  eapply safe_steps. eexact S1.
  apply (eval_simple_list_steps f k e m sq rargs vl E2 C'); auto.
...
(* builtin *)
  ...
  apply (eval_simple_list_steps f k e m sq rargs vl E C'); auto.
  simpl. intros X. exploit X. eapply rval_list_all_values.
  intros [vargs [t [vres [m' [U V]]]]].
  econstructor; econstructor; eapply step_builtin; eauto.
  eapply can_eval_simple_list; eauto. 
(* paren *)
  exploit (simple_can_eval_rval f k e m sq b (fun x => C(Eparen x ty))); eauto.
  intros [v1 [E1 S1]].
...
Remark deref_loc_trace:
  forall F V (ge: Genv.t F V) ty m sq b ofs t v,
  deref_loc ge ty m sq b ofs t v ->
...
Remark deref_loc_receptive:
  forall F V (ge: Genv.t F V) ty m sq b ofs ev1 t1 v ev2,
  deref_loc ge ty m sq b ofs (ev1 :: t1) v ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  t1 = nil /\ exists v', deref_loc ge ty m sq b ofs (ev2 :: nil) v'.
Proof.
  intros.
...
Remark assign_loc_trace:
  forall F V (ge: Genv.t F V) ty m sq b ofs t v m' sq',
  assign_loc ge ty m sq b ofs v t m' sq' ->
...
Remark assign_loc_receptive:
  forall F V (ge: Genv.t F V) ty m sq b ofs ev1 t1 v m' sq' ev2,
  assign_loc ge ty m sq b ofs v (ev1 :: t1) m' sq' ->
  match_traces ge (ev1 :: nil) (ev2 :: nil) ->
  ev1 :: t1 = ev2 :: nil.
Proof.
  intros.
...
(* receptiveness *)
  ...
  destruct (classic (exists t2', exists m'', exists sq'',
    assign_loc (Genv.globalenv p) (typeof l) m sq b ofs v4' t2' m'' sq'')).
  destruct H1 as [t2' [m'' [sq'' P]]]. 
  ...
  rewrite Heqo; rewrite Heqo0. intros; red; intros; elim H1. exists t0; exists m'0; exists sq'0; auto.
  econstructor; econstructor. left; eapply step_assignop_stuck with (v1 := v1'); eauto. 
  rewrite Heqo; rewrite Heqo0; auto.
  econstructor; econstructor. left; eapply step_assignop_stuck with (v1 := v1'); eauto. 
  rewrite Heqo; auto.
  ...
  econstructor; econstructor. left; eapply step_assignop with (v1 := v1'); eauto. simpl; reflexivity. 
  econstructor; econstructor. left; eapply step_assignop_stuck with (v1 := v1'); eauto. 
  rewrite Heqo; rewrite Heqo0. intros; red; intros; elim H1. exists t2; exists m'; exists sq'; auto.
  econstructor; econstructor. left; eapply step_assignop_stuck with (v1 := v1'); eauto. 
  rewrite Heqo; rewrite Heqo0; auto.
  econstructor; econstructor. left; eapply step_assignop_stuck with (v1 := v1'); eauto. 
  rewrite Heqo; auto.
  ...
  destruct (classic (exists t2', exists m'', exists sq'',
    assign_loc (Genv.globalenv p) (typeof l) m sq b ofs v3' t2' m'' sq'')).
  destruct H1 as [t2' [m'' [sq'' P]]]. 
...
Inductive eval_expression: env -> mem -> seqs -> expr -> trace -> mem -> seqs -> val -> Prop :=
  | eval_expression_intro: forall e m sq a t m' sq' a' v,
      eval_expr e m sq RV a t m' sq' a' ->
      eval_simple_rvalue ge e m' sq' a' v ->
      eval_expression e m sq a t m' sq' v

with eval_expr: env -> mem -> seqs -> kind -> expr -> trace -> mem -> seqs -> expr -> Prop :=
  | eval_val: forall e m sq v ty,
      eval_expr e m sq RV (Eval v ty) E0 m sq (Eval v ty)
  | eval_var: forall e m sq x ty,
      eval_expr e m sq LV (Evar x ty) E0 m sq (Evar x ty)
  | eval_field: forall e m sq a t m' sq' a' f ty,
      eval_expr e m sq RV a t m' sq' a' ->
      eval_expr e m sq LV (Efield a f ty) t m' sq' (Efield a' f ty)
  | eval_valof: forall e m sq a t m' sq' a' ty,
      type_is_volatile (typeof a) = false ->
      eval_expr e m sq LV a t m' sq' a' ->
      eval_expr e m sq RV (Evalof a ty) t m' sq' (Evalof a' ty)
  | eval_valof_volatile: forall e m sq a t1 m' sq' a' ty b ofs t2 v,
      type_is_volatile (typeof a) = true ->
      eval_expr e m sq LV a t1 m' sq' a' ->
      eval_simple_lvalue ge e m' sq' a' b ofs ->
      deref_loc ge (typeof a) m' sq' b ofs t2 v ->
      ty = typeof a ->
      eval_expr e m sq RV (Evalof a ty) (t1 ** t2) m' sq' (Eval v ty)
  | eval_deref: forall e m sq a t m' sq' a' ty,
      eval_expr e m sq RV a t m' sq' a' ->
      eval_expr e m sq LV (Ederef a ty) t m' sq' (Ederef a' ty)
  | eval_addrof: forall e m sq a t m' sq' a' ty,
      eval_expr e m sq LV a t m' sq' a' ->
      eval_expr e m sq RV (Eaddrof a ty) t m' sq' (Eaddrof a' ty)
  | eval_unop: forall e m sq a t m' sq' a' op ty,
      eval_expr e m sq RV a t m' sq' a' ->
      eval_expr e m sq RV (Eunop op a ty) t m' sq' (Eunop op a' ty)
  | eval_binop: forall e m sq a1 t1 m' sq' a1' a2 t2 m'' sq'' a2' op ty,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_expr e m' sq' RV a2 t2 m'' sq'' a2' ->
      eval_expr e m sq RV (Ebinop op a1 a2 ty) (t1 ** t2) m'' sq'' (Ebinop op a1' a2' ty)
  | eval_cast: forall e m sq a t m' sq' a' ty,
      eval_expr e m sq RV a t m' sq' a' ->
      eval_expr e m sq RV (Ecast a ty) t m' sq' (Ecast a' ty)
  | eval_seqand_true: forall e m sq a1 a2 ty t1 m' sq' a1' v1 t2 m'' sq'' a2' v2 v' v,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some true ->
      eval_expr e m' empty_seqs RV a2 t2 m'' sq'' a2' ->
      eval_simple_rvalue ge e m'' sq'' a2' v2 ->
      sem_cast v2 (typeof a2) type_bool = Some v' ->
      sem_cast v' type_bool ty = Some v ->
      eval_expr e m sq RV (Eseqand a1 a2 ty) (t1**t2) m'' sq'' (Eval v ty)
  | eval_seqand_false: forall e m sq a1 a2 ty t1 m' sq' a1' v1,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some false ->
      eval_expr e m sq RV (Eseqand a1 a2 ty) t1 m' sq' (Eval (Vint Int.zero) ty)
  | eval_seqor_false: forall e m sq a1 a2 ty t1 m' sq' a1' v1 t2 m'' sq'' a2' v2 v' v,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some false ->
      eval_expr e m' empty_seqs RV a2 t2 m'' sq'' a2' ->
      ...
      sem_cast v2 (typeof a2) type_bool = Some v' ->
      sem_cast v' type_bool ty = Some v ->
      eval_expr e m sq RV (Eseqor a1 a2 ty) (t1**t2) m'' sq'' (Eval v ty)
  | eval_seqor_true: forall e m sq a1 a2 ty t1 m' sq' a1' v1,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some true ->
      eval_expr e m sq RV (Eseqor a1 a2 ty) t1 m' sq' (Eval (Vint Int.one) ty)
  | eval_condition: forall e m sq a1 a2 a3 ty t1 m' sq' a1' v1 t2 m'' sq'' a' v' b v,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some b ->
      eval_expr e m' empty_seqs RV (if b then a2 else a3) t2 m'' sq'' a' ->
      eval_simple_rvalue ge e m'' sq'' a' v' ->
      sem_cast v' (typeof (if b then a2 else a3)) ty = Some v ->
      eval_expr e m sq RV (Econdition a1 a2 a3 ty) (t1**t2) m'' sq'' (Eval v ty)
  | eval_sizeof: forall e m sq ty' ty,
      eval_expr e m sq RV (Esizeof ty' ty) E0 m sq (Esizeof ty' ty)
  | eval_alignof: forall e m sq ty' ty,
      eval_expr e m sq RV (Ealignof ty' ty) E0 m sq (Ealignof ty' ty)
  | eval_assign: forall e m sq l r ty t1 m1 sq1 l' t2 m2 sq2 r' b ofs v v' t3 m3 sq3,
      eval_expr e m sq LV l t1 m1 sq1 l' ->
      eval_expr e m1 sq1 RV r t2 m2 sq2 r' ->
      eval_simple_lvalue ge e m2 sq2 l' b ofs ->
      eval_simple_rvalue ge e m2 sq2 r' v ->
      sem_cast v (typeof r) (typeof l) = Some v' ->
      assign_loc ge (typeof l) m2 sq2 b ofs v' t3 m3 sq3 ->
      ty = typeof l ->
      eval_expr e m sq RV (Eassign l r ty) (t1**t2**t3) m3 sq3 (Eval v' ty)
  | eval_assignop: forall e m sq op l r tyres ty t1 m1 sq1 l' t2 m2 sq2 r' b ofs
                          v1 v2 v3 v4 t3 t4 m3 sq3,
      eval_expr e m sq LV l t1 m1 sq1 l' ->
      eval_expr e m1 sq1 RV r t2 m2 sq2 r' ->
      eval_simple_lvalue ge e m2 sq2 l' b ofs ->
      deref_loc ge (typeof l) m2 sq2 b ofs t3 v1 ->
      eval_simple_rvalue ge e m2 sq2 r' v2 ->
      sem_binary_operation op v1 (typeof l) v2 (typeof r) m2 = Some v3 ->
      sem_cast v3 tyres (typeof l) = Some v4 ->
      assign_loc ge (typeof l) m2 sq2 b ofs v4 t4 m3 sq3 ->
      ty = typeof l ->
      eval_expr e m sq RV (Eassignop op l r tyres ty) (t1**t2**t3**t4) m3 sq3 (Eval v4 ty)
  | eval_postincr: forall e m sq id l ty t1 m1 sq1 l' b ofs v1 v2 v3 m2 sq2 t2 t3,
      eval_expr e m sq LV l t1 m1 sq1 l' ->
      eval_simple_lvalue ge e m1 sq1 l' b ofs ->
      deref_loc ge ty m1 sq1 b ofs t2 v1 ->
      sem_incrdecr id v1 ty = Some v2 ->
      sem_cast v2 (typeconv ty) ty = Some v3 ->
      assign_loc ge ty m1 sq1 b ofs v3 t3 m2 sq2 ->
      ty = typeof l ->
      eval_expr e m sq RV (Epostincr id l ty) (t1**t2**t3) m2 sq2 (Eval v1 ty)
  | eval_comma: forall (sp : bool) e m sq r1 r2 ty t1 m1 sq1 r1' v1 t2 m2 sq2 r2',
      eval_expr e m sq RV r1 t1 m1 sq1 r1' ->
      eval_simple_rvalue ge e m1 sq1 r1' v1 ->
      eval_expr e m1 (if sp then empty_seqs else sq1) RV r2 t2 m2 sq2 r2' ->
      ty = typeof r2 ->
      eval_expr e m sq RV (Ecomma sp r1 r2 ty) (t1**t2) m2 sq2 r2'

  | eval_call: forall e m sq rf rargs ty t1 m1 sq1 rf' t2 m2 sq2 rargs' vf vargs
                      targs tres cconv fd t3 m3 vres,
      eval_expr e m sq RV rf t1 m1 sq1 rf' -> eval_exprlist e m1 sq1 rargs t2 m2 sq2 rargs' ->
      eval_simple_rvalue ge e m2 sq2 rf' vf ->
      eval_simple_list ge e m2 sq2 rargs' targs vargs ->
      classify_fun (typeof rf) = fun_case_f targs tres cconv ->
      Genv.find_funct ge vf = Some fd ->
      type_of_fundef fd = Tfunction targs tres cconv ->
      eval_funcall m2 fd vargs t3 m3 vres ->
      eval_expr e m sq RV (Ecall rf rargs ty) (t1**t2**t3) m3 empty_seqs (Eval vres ty)

with eval_exprlist: env -> mem -> seqs -> exprlist -> trace -> mem -> seqs -> exprlist -> Prop :=
  | eval_nil: forall e m sq,
      eval_exprlist e m sq Enil E0 m sq Enil
  | eval_cons: forall e m sq a1 al t1 m1 sq1 a1' t2 m2 sq2 al',
      eval_expr e m sq RV a1 t1 m1 sq1 a1' ->
      eval_exprlist e m1 sq1 al t2 m2 sq2 al' ->
      eval_exprlist e m sq (Econs a1 al) (t1**t2) m2 sq2 (Econs a1' al')
...
with exec_stmt: env -> mem -> statement -> trace -> mem -> outcome -> Prop :=
  | exec_Sskip:   forall e m,
      exec_stmt e m Sskip
               E0 m Out_normal
  | exec_Sdo:     forall e m a t m' sq' v,
      eval_expression e m empty_seqs a t m' sq' v ->
      exec_stmt e m (Sdo a)
                t m' Out_normal
  ...
  | exec_Sseq_2:   forall e m s1 s2 t1 m1 out,
      ...
      exec_stmt e m (Ssequence s1 s2)
                t1 m1 out
  | exec_Sifthenelse: forall e m a s1 s2 t1 m1 sq1 v1 t2 m2 b out,
      eval_expression e m empty_seqs a t1 m1 sq1 v1 ->
  ...
  | exec_Sreturn_some: forall e m a t m' sq' v,
      eval_expression e m empty_seqs a t m' sq' v ->
  ...
  | exec_Scontinue:   forall e m,
      exec_stmt e m Scontinue
               E0 m Out_continue
  | exec_Swhile_false: forall e m a s t m' sq' v,
      eval_expression e m empty_seqs a t m' sq' v ->
      bool_val v (typeof a) = Some false ->
      exec_stmt e m (Swhile a s)
                t m' Out_normal
  | exec_Swhile_stop: forall e m a s t1 m1 sq1 v t2 m2 out' out,
      eval_expression e m empty_seqs a t1 m1 sq1 v ->
      bool_val v (typeof a) = Some true ->
      exec_stmt e m1 s t2 m2 out' ->
      out_break_or_return out' out ->
      exec_stmt e m (Swhile a s)
                (t1**t2) m2 out
  | exec_Swhile_loop: forall e m a s t1 m1 sq1 v t2 m2 out1 t3 m3 out,
      eval_expression e m empty_seqs a t1 m1 sq1 v ->
      bool_val v (typeof a) = Some true ->
      ...
      exec_stmt e m (Swhile a s)
                (t1 ** t2 ** t3) m3 out
  | exec_Sdowhile_false: forall e m s a t1 m1 out1 t2 m2 sq2 v,
      exec_stmt e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression e m1 empty_seqs a t2 m2 sq2 v ->
      bool_val v (typeof a) = Some false ->
  ...
  | exec_Sdowhile_stop: forall e m s a t m1 out1 out,
      ...
      exec_stmt e m (Sdowhile a s)
                t m1 out
  | exec_Sdowhile_loop: forall e m s a t1 m1 out1 t2 m2 sq2 v t3 m3 out,
      exec_stmt e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression e m1 empty_seqs a t2 m2 sq2 v ->
      bool_val v (typeof a) = Some true ->
  ...
  | exec_Sfor_start: forall e m s a1 a2 a3 out m1 m2 t1 t2,
      ...
      exec_stmt e m (Sfor a1 a2 a3 s) 
                (t1 ** t2) m2 out
  | exec_Sfor_false: forall e m s a2 a3 t m' sq' v,
      eval_expression e m empty_seqs a2 t m' sq' v ->
      bool_val v (typeof a2) = Some false ->
      exec_stmt e m (Sfor Sskip a2 a3 s)
                t m' Out_normal
  | exec_Sfor_stop: forall e m s a2 a3 t1 m1 sq1 v t2 m2 out1 out,
      eval_expression e m empty_seqs a2 t1 m1 sq1 v ->
      bool_val v (typeof a2) = Some true ->
      exec_stmt e m1 s t2 m2 out1 ->
      out_break_or_return out1 out ->
      exec_stmt e m (Sfor Sskip a2 a3 s)
                (t1 ** t2) m2 out
  | exec_Sfor_loop: forall e m s a2 a3 t1 m1 sq1 v t2 m2 out1 t3 m3 t4 m4 out,
      eval_expression e m empty_seqs a2 t1 m1 sq1 v ->
      bool_val v (typeof a2) = Some true ->
      exec_stmt e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
      ...
      exec_stmt e m (Sfor Sskip a2 a3 s)
                (t1 ** t2 ** t3 ** t4) m4 out
  | exec_Sswitch:   forall e m a sl t1 m1 sq1 n t2 m2 out,
      eval_expression e m empty_seqs a t1 m1 sq1 (Vint n) ->
...
CoInductive evalinf_expr: env -> mem -> seqs -> kind -> expr -> traceinf -> Prop :=
  | evalinf_field: forall e m sq a t f ty,
      evalinf_expr e m sq RV a t ->
      evalinf_expr e m sq LV (Efield a f ty) t
  | evalinf_valof: forall e m sq a t ty,
      evalinf_expr e m sq LV a t ->
      evalinf_expr e m sq RV (Evalof a ty) t
  | evalinf_deref: forall e m sq a t ty,
      evalinf_expr e m sq RV a t ->
      evalinf_expr e m sq LV (Ederef a ty) t
  | evalinf_addrof: forall e m sq a t ty,
      evalinf_expr e m sq LV a t ->
      evalinf_expr e m sq RV (Eaddrof a ty) t
  | evalinf_unop: forall e m sq a t op ty,
      evalinf_expr e m sq RV a t ->
      evalinf_expr e m sq RV (Eunop op a ty) t
  | evalinf_binop_left: forall e m sq a1 t1 a2 op ty,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Ebinop op a1 a2 ty) t1
  | evalinf_binop_right: forall e m sq a1 t1 m' sq' a1' a2 t2 op ty,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      evalinf_expr e m' sq' RV a2 t2 ->
      evalinf_expr e m sq RV (Ebinop op a1 a2 ty) (t1 *** t2)
  | evalinf_cast: forall e m sq a t ty,
      evalinf_expr e m sq RV a t ->
      evalinf_expr e m sq RV (Ecast a ty) t
  | evalinf_seqand: forall e m sq a1 a2 ty t1,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Eseqand a1 a2 ty) t1
  | evalinf_seqand_2: forall e m sq a1 a2 ty t1 m' sq' a1' v1 t2,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some true ->
      evalinf_expr e m' empty_seqs RV a2 t2 ->
      evalinf_expr e m sq RV (Eseqand a1 a2 ty) (t1***t2)
  | evalinf_seqor: forall e m sq a1 a2 ty t1,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Eseqor a1 a2 ty) t1
  | evalinf_seqor_2: forall e m sq a1 a2 ty t1 m' sq' a1' v1 t2,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some false ->
      evalinf_expr e m' empty_seqs RV a2 t2 ->
      evalinf_expr e m sq RV (Eseqor a1 a2 ty) (t1***t2)
  | evalinf_condition: forall e m sq a1 a2 a3 ty t1,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Econdition a1 a2 a3 ty) t1
  | evalinf_condition_2: forall e m sq a1 a2 a3 ty t1 m' sq' a1' v1 t2 b,
      eval_expr e m sq RV a1 t1 m' sq' a1' ->
      eval_simple_rvalue ge e m' sq' a1' v1 ->
      bool_val v1 (typeof a1) = Some b ->
      evalinf_expr e m' empty_seqs RV (if b then a2 else a3) t2 ->
      evalinf_expr e m sq RV (Econdition a1 a2 a3 ty) (t1***t2)
  | evalinf_assign_left: forall e m sq a1 t1 a2 ty,
      evalinf_expr e m sq LV a1 t1 ->
      evalinf_expr e m sq RV (Eassign a1 a2 ty) t1
  | evalinf_assign_right: forall e m sq a1 t1 m' sq' a1' a2 t2 ty,
      eval_expr e m sq LV a1 t1 m' sq' a1' ->
      evalinf_expr e m' sq' RV a2 t2 ->
      evalinf_expr e m sq RV (Eassign a1 a2 ty) (t1 *** t2)
  | evalinf_assignop_left: forall e m sq a1 t1 a2 op tyres ty,
      evalinf_expr e m sq LV a1 t1 ->
      evalinf_expr e m sq RV (Eassignop op a1 a2 tyres ty) t1
  | evalinf_assignop_right: forall e m sq a1 t1 m' sq' a1' a2 t2 op tyres ty,
      eval_expr e m sq LV a1 t1 m' sq' a1' ->
      evalinf_expr e m' sq' RV a2 t2 ->
      evalinf_expr e m sq RV (Eassignop op a1 a2 tyres ty) (t1 *** t2)
  | evalinf_postincr: forall e m sq a t id ty,
      evalinf_expr e m sq LV a t ->
      evalinf_expr e m sq RV (Epostincr id a ty) t
  | evalinf_comma_left: forall sp e m sq a1 t1 a2 ty,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Ecomma sp a1 a2 ty) t1
  | evalinf_comma_right: forall (sp : bool) e m sq a1 t1 m1 sq1 a1' v1 a2 t2 ty,
      eval_expr e m sq RV a1 t1 m1 sq1 a1' ->
      eval_simple_rvalue ge e m1 sq1 a1' v1 ->
      ty = typeof a2 ->
      evalinf_expr e m1 (if sp then empty_seqs else sq1) RV a2 t2 ->
      evalinf_expr e m sq RV (Ecomma sp a1 a2 ty) (t1 *** t2)
  | evalinf_call_left: forall e m sq a1 t1 a2 ty,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_expr e m sq RV (Ecall a1 a2 ty) t1
  | evalinf_call_right: forall e m sq a1 t1 m1 sq1 a1' a2 t2 ty,
      eval_expr e m sq RV a1 t1 m1 sq1 a1' -> 
      evalinf_exprlist e m1 sq1 a2 t2 ->
      evalinf_expr e m sq RV (Ecall a1 a2 ty) (t1 *** t2)
  | evalinf_call: forall e m sq rf rargs ty t1 m1 sq1 rf' t2 m2 sq2 rargs' vf vargs
      ...
      evalinf_expr e m sq RV (Ecall rf rargs ty) (t1***t2***t3)

with evalinf_exprlist: env -> mem -> seqs -> exprlist -> traceinf -> Prop :=
  | evalinf_cons_left: forall e m sq a1 al t1,
      evalinf_expr e m sq RV a1 t1 ->
      evalinf_exprlist e m sq (Econs a1 al) t1
  | evalinf_cons_right: forall e m sq a1 al t1 m1 sq1 a1' t2,
      eval_expr e m sq RV a1 t1 m1 sq1 a1' ->
      evalinf_exprlist e m1 sq1 al t2 ->
      evalinf_exprlist e m sq (Econs a1 al) (t1***t2)
...
with execinf_stmt: env -> mem -> statement -> traceinf -> Prop :=
  | execinf_Sdo:     forall e m a t,
      evalinf_expr e m empty_seqs RV a t ->
  ...
  | execinf_Sifthenelse_test: forall e m a s1 s2 t1,
      evalinf_expr e m empty_seqs RV a t1 ->
      execinf_stmt e m (Sifthenelse a s1 s2) t1
  | execinf_Sifthenelse: forall e m a s1 s2 t1 m1 sq1 v1 t2 b,
      eval_expression e m empty_seqs a t1 m1 sq1 v1 ->
      bool_val v1 (typeof a) = Some b ->
  ...
  | execinf_Swhile_body: forall e m a s t1 m1 sq1 v t2,
      eval_expression e m empty_seqs a t1 m1 sq1 v ->
      bool_val v (typeof a) = Some true ->
      execinf_stmt e m1 s t2 ->
      execinf_stmt e m (Swhile a s) (t1***t2)
  | execinf_Swhile_loop: forall e m a s t1 m1 sq1 v t2 m2 out1 t3,
      eval_expression e m empty_seqs a t1 m1 sq1 v ->
      bool_val v (typeof a) = Some true ->
      exec_stmt e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
  ...
  | execinf_Sdowhile_test: forall e m s a t1 m1 out1 t2,
      exec_stmt e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      evalinf_expr e m1 empty_seqs RV a t2 ->
      execinf_stmt e m (Sdowhile a s) (t1***t2)
  | execinf_Sdowhile_loop: forall e m s a t1 m1 out1 t2 m2 sq2 v t3,
      exec_stmt e m s t1 m1 out1 ->
      out_normal_or_continue out1 ->
      eval_expression e m1 empty_seqs a t2 m2 sq2 v ->
      bool_val v (typeof a) = Some true ->
  ...
  | execinf_Sfor_test: forall e m s a2 a3 t,
      evalinf_expr e m empty_seqs RV a2 t ->
      execinf_stmt e m (Sfor Sskip a2 a3 s) t
  | execinf_Sfor_body: forall e m s a2 a3 t1 m1 sq1 v t2,
      eval_expression e m empty_seqs a2 t1 m1 sq1 v ->
      bool_val v (typeof a2) = Some true ->
      execinf_stmt e m1 s t2 ->
      execinf_stmt e m (Sfor Sskip a2 a3 s) (t1***t2)
  | execinf_Sfor_next: forall e m s a2 a3 t1 m1 sq1 v t2 m2 out1 t3,
      eval_expression e m empty_seqs a2 t1 m1 sq1 v ->
      bool_val v (typeof a2) = Some true ->
      exec_stmt e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
  ...
  | execinf_Sfor_loop: forall e m s a2 a3 t1 m1 sq1 v t2 m2 out1 t3 m3 t4,
      eval_expression e m empty_seqs a2 t1 m1 sq1 v ->
      bool_val v (typeof a2) = Some true ->
      exec_stmt e m1 s t2 m2 out1 ->
      out_normal_or_continue out1 ->
  ...
  | execinf_Sswitch_body:   forall e m a sl t1 m1 sq1 n t2,
      eval_expression e m empty_seqs a t1 m1 sq1 (Vint n) ->
...
Inductive outcome_state_match
  ...
  | osm_return_some: forall v ty k' sq,
      call_cont k' = call_cont k ->
      outcome_state_match e m f k
        (Out_return (Some (v, ty))) (ExprState f (Eval v ty) (Kreturn k') e m sq).
...
Lemma bigstep_to_steps:
  (forall e m sq a t m' sq' v,
   eval_expression e m sq a t m' sq' v ->
   forall f k,
   star step ge (ExprState f a k e m sq) t (ExprState f (Eval v (typeof a)) k e m' sq'))
/\(forall e m sq K a t m' sq' a',
   eval_expr e m sq K a t m' sq' a' ->
   forall C f k, leftcontext K RV C ->
   simple a' = true /\ typeof a' = typeof a /\
   star step ge (ExprState f (C a) k e m sq) t (ExprState f (C a') k e m' sq'))
/\(forall e m sq al t m' sq' al',
   eval_exprlist e m sq al t m' sq' al' ->
   forall a1 al2 ty C f k, leftcontext RV RV C -> simple a1 = true -> simplelist al2 = true ->
   simplelist al' = true /\
   star step ge (ExprState f (C (Ecall a1 (exprlist_app al2 al) ty)) k e m sq)
              t (ExprState f (C (Ecall a1 (exprlist_app al2 al') ty)) k e m' sq'))
...
(* postincr *)
  ...
  traceEq.
(* comma *)
  exploit (H0 (fun x => C(Ecomma sp x r2 ty))).
    eapply leftcontext_compose; eauto. repeat constructor. intros [A [B D]].
...
Fixpoint esize (a: expr) : nat :=
  ...
  | Ecomma _ r1 r2 _ => S(esize r1 + esize r2)%nat
...
Proof.
  ...
  assert (COE:
    forall e m sq K a t C f k,
    evalinf_expr e m sq K a t ->
    leftcontext K RV C ->
    forever_N step lt ge (esize a) (ExprState f (C a) k e m sq) t).
  cofix COE.

  assert (COEL:
    forall e m sq a t C f k a1 al ty,
    evalinf_exprlist e m sq a t ->
    leftcontext RV RV C -> simple a1 = true -> simplelist al = true ->
    forever_N step lt ge (esizelist a)
                   (ExprState f (C (Ecall a1 (exprlist_app al a) ty)) k e m sq) t).
  cofix COEL.
  intros. inv H.
(* cons left *)
...
(* cons right *) 
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H3 
...
(* binop left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* binop right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Ebinop op x a2 ty)) f k)
  as [P [Q R]]. 
...
(* seqand left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* seqand 2 *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Eseqand x a2 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
...
(* seqor left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* seqor 2 *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Eseqor x a2 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_plus. eapply plus_right. eexact R. 
...
(* condition top *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* condition *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Econdition x a2 a3 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_plus. eapply plus_right. eexact R. 
...
(* assign left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* assign right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Eassign x a2 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_star with (a2 := (esize a2)). eexact R. simpl; omega.
...
(* assignop left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* assignop right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Eassignop op x a2 tyres ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_star with (a2 := (esize a2)). eexact R. simpl; omega.
...
(* comma left *)
  eapply forever_N_star with (a2 := (esize a1)). apply star_refl. simpl; omega.
  eapply COE with (C := fun x => C(Ecomma sp x a2 ty)). eauto. 
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* comma right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Ecomma sp x a2 (typeof a2))) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_plus. eapply plus_right. eexact R. 
...
(* call left *)
  ...
  eapply leftcontext_compose; eauto. repeat constructor. traceEq.
(* call right *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Ecall x a2 ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  eapply forever_N_star with (a2 := (esizelist a2)). eexact R. simpl; omega.
  eapply COEL with (al := Enil). eauto. auto. auto. auto. traceEq.
(* call *)
  destruct (eval_expr_to_steps _ _ _ _ _ _ _ _ _ H1 (fun x => C(Ecall x rargs ty)) f k)
  as [P [Q R]]. 
  eapply leftcontext_compose; eauto. repeat constructor.
  destruct (eval_exprlist_to_steps _ _ _ _ _ _ _ _ H2 rf' Enil ty C f k)
